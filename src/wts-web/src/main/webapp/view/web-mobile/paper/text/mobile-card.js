$(function() {
	initSideBoxSize();
	// 打开导航窗口
	$('#timeNavButtonId').click(function() {
		$('#navPaper-win').modal('show');
	})
	// 导航到元素下
	...
	$('#submitPaper-win').on(
			...
			function(e) {
				// 计算当前一共有多少道题，完成多少道题
				$("#card-finish-numinfo-all").text(
						$('.wts-side-subjuct-unit').size());
				$("#card-finish-numinfo-compelet").text(
						$('.wts-side-subjuct-unit.active').size());
			})
	// 启动页面倒计时脚本
	CountDownStart();
...
function CountDownStart() {
	...
	if ($('#' + timerBoxId).length > 0) {
		...
		if (maxtime > 1) {
			countDownTimer = setInterval("CountDown('" + timerBoxId + "')",
					1000);
		}
	}
}
...
function gotoTargetDom(domObj) {
	...
	$('#' + $(domObj).attr('targetId') + ' .subjectUnitViewBox').addClass(
			"active");
	$('#navPaper-win').modal('hide');
}
//滚动到提交按钮处
function gotoSubmitForm() {
	$('html,body').animate({
		scrollTop : $('#saveButtonsId').offset().top - 70
	}, 500);
	$('#navPaper-win').modal('hide');
}