						...
						<div>
							<div style="overflow: hidden;">
								<c:if test="${test.ALLNUM>1}">
									...
									<div style="margin: 4px;">
										<div class="side_unit_info"
											style="font-size: 14px; font-weight: 700; margin-bottom: 8px; line-height: 1.2em;">
											第${index>test.ALLNUM?test.ALLNUM:index}题&nbsp;/&nbsp;共${test.ALLNUM}题
										...
										<div class="side_unit_info">
											...
											《<b>${test.TITLE}</b>》&nbsp;</c:if>
											<code title="题型">${subjectu.tipType.title}</code>
											/
											<code title="分类">${subjectu.subjectType.name}</code>
											/
											<code>答题量${subjectu.subject.donum}人次</code>
											/
											<code>
												正确率
												<fmt:formatNumber type="number"
													value="${subjectu.subject.rightnum/subjectu.subject.donum*100}"
													maxFractionDigits="0">
												</fmt:formatNumber>
												%
											</code>
								...
								<c:if test="${test.ALLNUM==1}">
									...
									<div style="margin: 4px;">
										<div class="side_unit_info"
											style="font-size: 14px; font-weight: 700; margin-bottom: 8px; line-height: 1.5em;">
			...
			<div class="row" style="min-height: 200px;">
				<div class="container wts-paper-forms" style="padding: 0px;">
					...
					<div id="${subjectu.version.id}-NAVI">
						...
						<c:if test="${index>test.ALLNUM}">
							<div
								style="margin-top: 20px; padding: 20px; background-color: #ffffff; border: 1px dashed #ccc;">
								...
								<c:if test="${USEROBJ!=null }">
									<button type="button" class="btn btn-default"
										...
										<i id="book-y" class="glyphicon glyphicon-star"></i> <i
											id="book-n" class="glyphicon glyphicon-star-empty"></i>收藏(<span
											id="bookNumId">0</span>)
									</button>
									<button type="button" class="btn btn-default"
										onclick="doPraise()">
									...
									<a type="button" class="btn btn-default"
										...
										style="color: #000000;" class="glyphicon glyphicon-comment"></i>评(<span
