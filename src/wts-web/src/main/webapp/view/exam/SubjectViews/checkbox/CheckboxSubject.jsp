...
<script type="text/javascript">
	...
	function rightAnswer() {
		rightAndWrongAnswer(true);
	}
	function wrongAnswer() {
		rightAndWrongAnswer(false);
	}
	//选择正确或错误答案
	function rightAndWrongAnswer(isTrue) {
		var selectedArray = $(gridSubjectanswer).datagrid('getSelections');
		if (selectedArray.length > 0) {
			// 有数据执行操作
			var str = "该选项将被设置为" + (isTrue ? "正确" : "错误") + "答案！";
			$.messager.confirm(MESSAGE_PLAT.PROMPT, str, function(flag) {
				if (flag) {
					$(gridSubjectanswer).datagrid('loading');
					$.post('subjectanswer/rightVal.do?flag='
							+ (isTrue ? "1" : "0") + '&ids='
							+ $.farm.getCheckedIds(gridSubjectanswer, 'ID'),
							{}, function(flag) {
								var jsonObject = JSON.parse(flag, null);
