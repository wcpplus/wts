 ...
 */
@Repository
public class MessagesenderDaoImpl  extends HibernateSQLTools<Messagesender> implements  MessagesenderDaoInter {
  ...
  public Messagesender getEntity(String messagesenderid) {
    // TODO 自动生成代码,修改后请去除本注释
    Session session= sessionFatory.getCurrentSession();
    return (Messagesender)session.get(Messagesender.class, messagesenderid);
  }
  @Override
  public Messagesender insertEntity(Messagesender messagesender) {
    // TODO 自动生成代码,修改后请去除本注释
    Session session= sessionFatory.getCurrentSession();
    session.save(messagesender);
