...
public class RandomStepController extends WebUtils {
	private final static Logger log = Logger.getLogger(RandomStepController.class);
	@Resource
	...
	public Map<String, Object> editSubmit(RandomStep entity, HttpSession session) {
		...
		try {
			entity = randomStepServiceImpl.editRandomstepEntity(entity, getCurrentUser(session));
	...
	public Map<String, Object> delSubmit(String ids, HttpSession session) {
		try {
			for (String id : parseIds(ids)) {
				randomStepServiceImpl.deleteRandomstepEntity(id, getCurrentUser(session));
			}
	...
	public ModelAndView index(HttpSession session) {
		return ViewMode.getInstance().returnModelAndView("exam/RandomstepResult");
	}
		...
		try {
			...
			case (0): {// 查看
				return ViewMode.getInstance().putAttr("pageset", pageset)
						.putAttr("entity", randomStepServiceImpl.getRandomstepEntity(ids))
			...
			case (2): {// 修改
				RandomStep step = randomStepServiceImpl.getRandomstepEntity(ids);
				SubjectType type = subjectTypeServiceImpl.getSubjecttypeEntity(step.getTypeid());
				return ViewMode.getInstance().putAttr("pageset", pageset).putAttr("typename", type.getName())
						.putAttr("entity", step).returnModelAndView("exam/RandomstepForm");
			}
			default:
				break;
			}
			return ViewMode.getInstance().returnModelAndView("exam/RandomstepForm");
		} catch (Exception e) {
			return ViewMode.getInstance().setError(e + e.getMessage(), e).returnModelAndView("exam/RandomstepForm");
		}
	}
}
